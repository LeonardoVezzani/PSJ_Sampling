...
/*PSJ - Point spread jitter: to know more about this pattern check the related Paper   [WIP]
 *By Leonardo Vezzani
 *Contacts:
...
ccl_device void cmj_sample_2D(int s, int N, int p, float *fx, float *fy)
{
  ...
  float R = 20.0f; 
  float peak = 0.10f;     
  float firstAiry = 0.30f;
  float secondAiry = 0.40f;
  float secondaryAiryEnd = 0.48f;
  float mag = 0.0f;
  float ffx = 0.0f;
  float ffy = 0.0f;
  ...
  ffx = (sx + (sy + jx) * invn) * invm;
  ffy = (s + jy) * invN;
  mag = sqrt(pow((ffx), 2) + pow((ffy ), 2));
  
  if (mag > peak && mag < firstAiry) {
    *fx = r * ffx;  // *fy = ( r * (ffy - 0.5f)) + 0.5f;
    *fy = r * ffy;  // *fx = ( r * (ffx - 0.5f)) + 0.5f;
  }
  else if (mag > secondAiry && mag < secondaryAiryEnd) {
    *fx = R * ffx;  // *fx = ( R * (ffx - 0.5f)) + 0.5f;
    *fy = R * ffy;  // *fy = ( R * (ffy - 0.5f)) + 0.5f;
  }
  else {
    *fx = ffx;
    *fy = ffy;
